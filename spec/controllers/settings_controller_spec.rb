...
RSpec.describe SettingsController do
  ...
  describe '#pushover_auth' do
    ...
    context 'when Pushover is enabled' do
      it 'redirects to Pushover subscription URL' do
        ...
        allow(Pushover).to receive(:subscription_url).and_return("http://pushover.com/subscribe")
        get :pushover_auth
        expect(session[:pushover_rand]).to eq("random_token")
        expect(response).to redirect_to("http://pushover.com/subscribe")
      end
    end
  end
  ...
  describe '#mastodon_auth' do
    context 'when Mastodon app is persisted' do
      it 'redirects to Mastodon OAuth URL' do
        app = double("MastodonApp", persisted?: true, oauth_auth_url: "http://mastodon.com/oauth")
        allow(MastodonApp).to receive(:find_or_register).and_return(app)
        get :mastodon_auth, params: { mastodon_instance_name: "instance" }
        expect(response).to redirect_to("http://mastodon.com/oauth")
      end
    end
  ...
  describe '#github_auth' do
    it 'redirects to GitHub OAuth URL' do
      allow(SecureRandom).to receive(:hex).and_return("random_state")
      allow(Github).to receive(:oauth_auth_url).and_return("http://github.com/oauth")
      get :github_auth
      expect(session[:github_state]).to eq("random_state")
      expect(response).to redirect_to("http://github.com/oauth")
    end
  end
end