...
describe ApplicationHelper do
  ...
  describe "#filtered_tags" do
    ...
    let(:cookies) { { ApplicationController::TAG_FILTER_COOKIE => "tag3,tag4" } }
    ...
    it "returns cookie-based tags if user is not present" do
      assign(:user, nil)
      allow(Tag).to receive(:where).and_return(["tag3", "tag4"])
  ...
  describe "#inline_avatar_for" do
    ...
    it "returns a link to the user's avatar if viewer is present and shows avatars" do
      result = helper.inline_avatar_for(viewer, user)
  ...
  describe "#link_to_different_page" do
    ...
    it "returns a link with the correct class when on a different page" do
      ...
      expect(result).to_not include("current_page")
    end

    it "returns a link with the current_page class when on the same page" do
      result = helper.link_to_different_page("Current", "/current/page")
      expect(result).to include("a")
      expect(result).to include("href=\"/current/page\"")
      expect(result).to include("current_page")
    end
  end
  ...
  describe "#possible_flag_warning" do
    ...
    it "does not render the flag warning partial if not in production" do
      ...
      expect(helper).to_not receive(:render).with(partial: "users/flag_warning", locals: { showing_user: showing_user, interval: { param: "1m" } })
      helper.possible_flag_warning(showing_user, user)
    end
  end

  describe "#tag_link" do
    let(:tag) { double("Tag", tag: "tag_name", css_class: "tag-class", description: "Tag description") }
    ...
    it "returns a link to the tag with the correct attributes" do
      ...
      expect(result).to include("a")
      expect(result).to include("href=\"/tags/tag_name\"")
  ...
  describe "#how_long_ago_label" do
    ...
    it "returns a time tag with the correct attributes" do
      allow(helper).to receive(:how_long_ago).and_return("5 minutes ago")
      result = helper.how_long_ago_label(time)
      expect(result).to include("time")
      expect(result).to include("5 minutes ago")
  ...
  describe "#how_long_ago_link" do
    ...
    it "returns a link with the correct attributes" do
      allow(helper).to receive(:how_long_ago_label).and_return("5 minutes ago")
