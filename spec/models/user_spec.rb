...
class User < ApplicationRecord
  ...
  def underscores_and_dashes_in_username
    # Skip check for blank usernames; other validations will handle invalid values.
    return if username.blank?

    # Escape any regex meta characters to avoid unintended patterns or driver issues,
    # then replace underscores/dashes with a character class so they collide.
    username_regex = "^" + Regexp.escape(username).gsub(/_|-/, "[-_]") + "$"
